...
class RPGAgent(BaseAgent):
    ...
    def compute_reward_bonus(self, obs, actions = None):
        ...
        # width on critic network
            ...
            # print(f'========= The Width Bonus is : {reward_bonus} ==========')
    ...
    def width_loss(self, size, output, target):
        ...
        # print(f'=========== Width Loss is {loss} ===========')
        return loss

    def retrain_width(self, mode):
        print("=========== Retraining width! ============")
        ...
        for _ in range(self.config.width_loop):
            ...
            if self.config.bonus_select == 2:
                # print(query_pool.shape, query_batch)
    ...
    def gather_trajectories(self, roll_in=True, add_bonus_reward=True, debug=False, mode=None, record_return=False):
        ...
        # First, update the bonus coeff
        if self.delay > 1:
            self.config.bonus_coeff = 1 / 2 ** self.iterations
            # self.config.bonus_coeff = self.config.bonus_coeff
        ...
        for i in range(roll_out_length):
            ...
            if add_bonus_reward:
                ...
                # print('[Checking if we need to retrain the width function!]')
                if self.config.bonus == 'width' and self.query_counter < (self.config.query_size + 1):
                    # print('[Yes! We either add data to the query pool or retrain the width function]')
                    if self.config.online: 
                        # print('[We are in the online mode, directly pretrain the width!]')
                        self.query_pool = np.copy(s)
                        if self.iterations % self.delay == 0:
                            self.retrain_width(mode = 'explore')
                    else:
                        # print('[We are not in the online mode!]')
                        if not self.query_counter:
                            # print('[Query counter is missing, simply copy the states]')
                            self.query_pool = np.copy(s)
                        elif self.query_counter < self.config.query_size and np.random.sample() <= (1 / self.delay):
                            # print('Adding samples to query pool')
                            self.query_pool = np.append(self.query_pool, s, axis=0)
                            # self.query_counter += self.config.num_workers
                        self.query_counter += self.config.num_workers
                        # self.query_counter += 1
                            
                        if self.query_counter and self.query_counter >= self.config.query_size: # after cumulating enough queries, retrain width
                            print('[Retrain Width!]')
                            self.retrain_width(mode = 'explore')
